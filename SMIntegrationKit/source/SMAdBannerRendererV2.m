...
@implementation SMAdBannerRendererV2
...
- (id) init {
	...
	_containervc = [[SMAdViewController alloc] init];
...
- (void) updateFramesForTakeover {
	...
	//update ad content
	if(isportrait && takeoverPortrait) [_webview stringByEvaluatingJavaScriptFromString:@"showTakeoverPortrait()"];
	else if(!isportrait && takeoverLandscape) [_webview stringByEvaluatingJavaScriptFromString:@"showTakeoverLandscape()"];
	else if(isportrait && !takeoverPortrait && takeoverLandscape) [_webview stringByEvaluatingJavaScriptFromString:@"showTakeoverLandscape()"];
	else if(!isportrait && !takeoverLandscape) [_webview stringByEvaluatingJavaScriptFromString:@"showTakeoverPortrait()"];
	else [_webview stringByEvaluatingJavaScriptFromString:@"showTakeoverPortrait()"];
}
...
- (void) dealloc {
	...
	NSLog(@"DEALLOC: SMAdBannerRendererV2");
	#endif
	[last release];
